...
//	
//
//  Source file: /horowitz/users/dhuff/CGRAGenerator/hardware/generator_z/memory_core/linebuffer_control.vp
...
always @(posedge clk or posedge reset) begin
  ...
  //We can alter the read and write addresses in the line buffer
  else if (config_en==1'b1) begin
    case(config_addr[31:24])
      8'd0: read_addr <= config_data[15:0];
      8'd1: write_addr <= config_data[15:0];
    endcase 	
  end 
  else if (fsm_en==1'b1) begin
    if (phase==1'b1) begin 
  ...
  if (reset==1'b1) begin
    ...
    if (phase==1'b0) begin
      ...
      if ((read_state == 2'd0) && (stall_read==1'b0) && (stall_read_int==1'b0)) begin //00 = not reading
        ...
        if ((stall_read==1'b0) && (stall_read_int==1'b0)) begin
          read_state <= 2'd1;
        end else begin
          read_state <= 2'd0;
          delay_outputs <= 1'b0;
...
always @(*) begin
  //if (phase==1'b1) begin  // ?? ??? dunno why FIXME
  if (phase==1'b0) begin
  ...
  //We can read the read and write addresses in the line buffer
always @(*) begin
  case(config_addr[31:24])
